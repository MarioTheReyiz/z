...
/**
 * AUTO-GENERATED by gen_hardcoded.py — DO NOT EDIT MANUALLY
 * mappings.txt kaynaklı hardcoded mapping'ler.
 * Main.java'da scanAndMap() SONRASINDA çağır.
 */
public class HardcodedUtils {
    ...
    public static void apply() {
        ...
        // Classlar zaten scanAndMap() tarafından AutoMapper'a yüklenmiş durumda.
        // CLASS_MAP loopunu atla — sadece eksik olanları tamamla.
        // FIELD ve METHOD loopları AutoMapper.get(alias) ile direkt çalışır.

        int loaded = 0;
        int classesLoaded = 0;

        // Sadece AutoMapper'da henüz olmayan classları ekle
        ...
        if (classes != null && !classes.isEmpty()) {
            ...
            for (Map.Entry<String, String> e : CLASS_MAP.entrySet()) {
                ...
                if (AutoMapper.get(alias) != null) continue; // zaten var
        ...
        for (Map.Entry<String, String> e : FIELD_MAP.entrySet()) {
            ...
            if (AutoMapper.getField(e.getValue()) != null) continue; // zaten var

            Class<?> c = AutoMapper.get(classAlias);
            if (c == null) {
                me.pewa.util.Logger.warn("[HC] FIELD SKIP (no class): " + e.getValue());
                continue;
            }
            Field f = findField(c, obfField);
            if (f == null) {
                me.pewa.util.Logger.warn("[HC] FIELD NOT FOUND: " + c.getName() + "." + obfField + " (" + e.getValue() + ")");
            } else {
        ...
        for (Map.Entry<String, String> e : METHOD_MAP.entrySet()) {
            ...
            if (AutoMapper.getMethod(e.getValue()) != null) continue; // zaten var

            Class<?> c = AutoMapper.get(classAlias);
            if (c == null) {
                me.pewa.util.Logger.warn("[HC] METHOD SKIP (no class): " + e.getValue());
                continue;
            }
            Method m = findMethod(c, obfMethod);
            if (m == null) {
                me.pewa.util.Logger.warn("[HC] METHOD NOT FOUND: " + c.getName() + "." + obfMethod + " (" + e.getValue() + ")");
            } else {
        ...
        me.pewa.util.Logger.info("[HardcodedUtils] Applied " + (loaded + classesLoaded)
                + " hardcoded mappings (classes=" + classesLoaded + ", fields+methods=" + loaded + ").");
    }
